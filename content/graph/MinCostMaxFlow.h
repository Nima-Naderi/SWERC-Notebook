...
struct MCMF { 
    ...
    vi seen;
    vector<EDGE_W> dist, pi;
    vector<edge*> par;

    MCMF(int N) : N(N), ed(N), seen(N), dist(N), pi(N), par(N) {}

    void addEdge(int from, int to, ll cap, EDGE_W cost) { 
        if (from == to) return;
    ...
    void path(int s) { 
        ...
        __gnu_pbds::priority_queue<pair<EDGE_W, int>> q;
    ...
    pair<ll, EDGE_W> maxflow(int s, int t) {
        ll totflow = 0;
        EDGE_W totcost = 0;
        while (path(s), seen[t]) {
            ll fl = INF;
